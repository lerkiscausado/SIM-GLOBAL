...
Namespace Controles
    Public Class DImagenes
        ...
        ReadOnly Funciones As New Funciones
        Public Shared Function Cargar(ByVal filtro As String) As Imagenes
            Try
        ...
        Public Function ListarImagenesActivasIC(ByVal IdDetalleOrden As String) As DataSet
            Try
                Dim query As String =
              String.Format("SELECT ruta AS RUTA,id,texto FROM imagenes WHERE id_detalle_orden='" & IdDetalleOrden & "' and estado='A'")
                _conn = ConexionODBC.Open()
                Dim comando = New OdbcCommand(query, _conn)
                _adapter = New OdbcDataAdapter(comando)
                _ds = New DataSet()
