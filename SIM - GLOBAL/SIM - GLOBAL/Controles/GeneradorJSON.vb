...
Public Class GeneradorJSON
    ...
    Public Function GenerarJSON(idFactura As String, numDocumentoIdObligado As String, numFactura As String) As String
        ...
        While dr1.Read()
            ...
            If Not usuariosDict.ContainsKey(numDoc) Then
                ...
                usuario("fechaNacimiento") = Newtonsoft.Json.Linq.JToken.FromObject(FormatearFecha(dr1("fechaNacimiento")))
                ...
                usuario("codZonaTerritorialResidencia") = Newtonsoft.Json.Linq.JToken.FromObject(If(dr1.IsDBNull(dr1.GetOrdinal("codZonaTerritorialResidencia")), String.Empty, dr1("codZonaTerritorialResidencia").ToString()))
                'usuario("codZonaTerritorialResidencia") = If(dr1.IsDBNull(dr1.GetOrdinal("codZonaTerritorialResidencia")), String.Empty, CType(dr1("codZonaTerritorialResidencia").ToString(), Object))
            ...
            Dim sql2 As String = "
                    SELECT '130010244901' AS codPrestador, 
                        ...
                        NULL AS codDiagnosticoRelacionadoCIE11,
            ...
            While dr2.Read()
                If tipoServicio = "P" Then
                    ...
                    proc("fechaInicioAtencion") = Newtonsoft.Json.Linq.JToken.FromObject(FormatearFechaHora(dr2("fechaInicioAtencion")))
                    ...
                    proc("codServicio") = Newtonsoft.Json.Linq.JToken.FromObject(If(IsNumeric(dr2("codServicio")), CInt(dr2("codServicio")), 0))
                    ...
                    proc("codDiagnosticoPrincipalCIE11") = Nothing
                    proc("nomCodDiagnosticoPrincipalCIE11") = Nothing
                    proc("codDiagnosticoRelacionado") = Nothing
                    proc("codDiagnosticoRelacionadoCIE11") = Nothing
                ...
                ElseIf tipoServicio = "C" Then
                    ...
                    cons("codDiagnosticoPrincipalCIE11") = Nothing
                    cons("nomCodDiagnosticoPrincipalCIE11") = Nothing
                    cons("codDiagnosticoRelacionado1") = Nothing
                    cons("codDiagnosticoRelacionado1CIE11") = Nothing
                    cons("nomCodDiagnosticoRelacionado1CIE11") = Nothing
                    cons("codDiagnosticoRelacionado2") = Nothing
                    cons("codDiagnosticoRelacionado2CIE11") = Nothing
                    cons("nomCodDiagnosticoRelacionado2CIE11") = Nothing
                    cons("codDiagnosticoRelacionado3") = Nothing
                    cons("codDiagnosticoRelacionado3CIE11") = Nothing
                    cons("nomCodDiagnosticoRelacionado3CIE11") = Nothing
    ...
    Public Function GenerarJSON2(idFactura As String, numDocumentoIdObligado As String, numFactura As String) As String
        Dim root As New JObject()
        root("numDocumentoIdObligado") = numDocumentoIdObligado
        root("numFactura") = numFactura
        root("tipoNota") = Nothing
        ...
        Using cmd1 As New OdbcCommand(sql1, _conn)
            cmd1.Parameters.AddWithValue("?", idFactura)
            Using da As New OdbcDataAdapter(cmd1)
                da.Fill(dtFactura) ' Cargamos todo en memoria para liberar la conexión
            End Using
        End Using

        For Each row1 As DataRow In dtFactura.Rows
            Dim idOrden As String = row1("idOrden").ToString()
            Dim tipoServicio As String = row1("tipo").ToString().ToUpper()
            Dim numDoc As String = row1("numDocumentoIdenfificacion").ToString()

            If Not usuariosDict.ContainsKey(numDoc) Then
                Dim usuario As New JObject()
                usuario("tipoDocumentoIdentificacion") = row1("tipoDocumentoIdentificacion").ToString()
                usuario("numDocumentoIdentificacion") = numDoc
                usuario("tipoUsuario") = row1("tipoUsuario").ToString()
                usuario("fechaNacimiento") = FormatearFecha(row1("fechaNacimiento"))
                usuario("codSexo") = row1("codSexo").ToString()
                usuario("codPaisResidencia") = row1("codPaisResidencia").ToString()
                usuario("codMunicipioResidencia") = row1("codMunicipioResidencia").ToString()
                usuario("codZonaTerritorialResidencia") = If(IsDBNull(row1("codZonaTerritorialResidencia")), Nothing, row1("codZonaTerritorialResidencia").ToString())
                usuario("incapacidad") = row1("incapacidad").ToString()
                usuario("consecutivo") = Convert.ToInt32(row1("consecutivo"))
                usuario("codPaisOrigen") = row1("codPaisOrigen").ToString()
            ...
            Using cmd2 As New OdbcCommand(sql2, _conn)
                cmd2.Parameters.AddWithValue("?", idOrden)
                Using dr2 As OdbcDataReader = cmd2.ExecuteReader()
                    While dr2.Read()
                        If tipoServicio = "P" Then
                            Dim proc As New JObject()
                            proc("codPrestador") = dr2("codPrestador").ToString()
                            ...
                            proc("vrServicio") = If(IsDBNull(dr2("valorServicio")), 0, CDec(dr2("valorServicio")))
                            proc("conceptoRecaudo") = dr2("conceptoRecaudo").ToString()
                        ...
                        ElseIf tipoServicio = "C" Then
                            ...
                            cons("vrServicio") = If(IsDBNull(dr2("valorServicio")), 0, CDec(dr2("valorServicio")))
                            cons("conceptoRecaudo") = dr2("conceptoRecaudo").ToString()
    ...
    Private Function FormatearFecha(valor As Object) As String
        ' Si es nulo, devolvemos una cadena vacía (o Nothing, pero tipado como String)
        If IsDBNull(valor) OrElse valor Is Nothing Then Return String.Empty

        Dim fecha As DateTime
        If DateTime.TryParse(valor.ToString(), fecha) Then
            ' El formato AAAA-MM-DD es el estándar correcto para RIPS JSON
