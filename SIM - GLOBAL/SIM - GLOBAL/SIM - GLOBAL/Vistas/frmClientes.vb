...
Public Class frmClientes
    ...
    Private Sub Guardar()
        ...
        _Clientes.Contacto = txtContacto.Text
    ...
    Private Sub EditarCliente()
        ...
        _Clientes = _DClientes.Cargar(_ClickIdCliente)
        txtCodigo.Text = _Clientes.Id
        cboTipoIdentificacion.ItemIndex = cboTipoIdentificacion.Properties.GetDataSourceRowIndex("CODIGO", _Clientes.IdTipoIdentificacion)
        txtNumeroIdentificacion.Text = _Clientes.Identificacion
        txtNombre.Text = _Clientes.Nombre
        txtDireccion.Text = _Clientes.Direccion
        txtTelefono.Text = _Clientes.Telefono
        txtCorreoElectronico.Text = _Clientes.CorreoElectronico
        txtContacto.Text = _Clientes.Contacto

        If _Clientes.Estado = "A" Then
            chkEstado.Checked = True
        Else
    ...
    Private Sub bbiGuardar_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiGuardar.ItemClick
        If txtNumeroIdentificacion.Text = "" Then
            MessageBox.Show("El Campo Identificación es Obligatorio", "Registro de Clientes", MessageBoxButtons.OK, MessageBoxIcon.Information)
        ElseIf txtNombre.Text = "" Then
            MessageBox.Show("El Campo Razon Social es Obligatorio", "Registro de Clientes", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
            Guardar()
    ...
    Private Sub bbiEditar_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiEditar.ItemClick
        If _ClickIdCliente = Nothing Then
            MessageBox.Show("Debe seleccionar un Cliente", "Registro de Clientes", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
            EditarCliente()
        End If
    End Sub
    ...
    Private Sub GVConsultar_RowClick(sender As Object, e As DevExpress.XtraGrid.Views.Grid.RowClickEventArgs) Handles GVConsultar.RowClick
        If e.RowHandle >= 0 Then
            _ClickIdCliente = GVConsultar.GetRowCellValue(e.RowHandle.ToString, "ID").ToString()
            _Fila = e.RowHandle.ToString
        End If
    End Sub

    Private Sub txtNumeroIdentificacion_Leave(sender As Object, e As EventArgs) Handles txtNumeroIdentificacion.Leave
        If _DClientes.Existe(cboTipoIdentificacion.GetColumnValue("CODIGO"), txtNumeroIdentificacion.Text) = True Then
            MessageBox.Show("Ya existe un cliente registrado con este numero", "Registro de Clientes", MessageBoxButtons.OK, MessageBoxIcon.Information)
