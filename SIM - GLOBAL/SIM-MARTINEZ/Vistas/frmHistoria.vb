...
Imports SIM_MARTINEZ.Controles
Imports SIM_MARTINEZ.Modelo
...
#Region "DECLARACION DE VARIABLES"
    ...
    Dim _dEscalaPrader As New SIM___GLOBAL.Controles.DEscalaPrader
    ...
    Dim _HistoriaDetalle As New SIM___GLOBAL.Modelo.HistoriaDetalle
    Dim _DHistoriaDetalle As New SIM___GLOBAL.Controles.DHistoriaDetalle
...
#Region "PROCEDIMIENTOS"
    Private Sub GraficaPaciente(ByVal Filtro As String)
        ' GRAFICA DE TALLA
        ' MAXIMO
        _ds = New DataSet
        _ds = _DPercentil.Paciente(Filtro, "TALLA")
        Dim seriesT As New Series("Talla", ViewType.Point)
        CCPercentilTalla.Series.Add(seriesT)
        ' Generate a data table and bind the series to it.
        seriesT.DataSource = _ds.Tables(0)
        seriesT.ArgumentScaleType = ScaleType.Numerical
        seriesT.ArgumentDataMember = "Argument"
        seriesT.ValueScaleType = ScaleType.Numerical
        seriesT.ValueDataMembers.AddRange(New String() {"Value"})
        seriesT.View.Color = Color.Black

        '        CCPercentilTalla.Series(4).Points(1).Color = Color.Red

        ' GRAFICA DE PESO
        ' MAXIMO
        _ds = New DataSet
        _ds = _DPercentil.Paciente(Filtro, "PESO")
        Dim seriesP As New Series("Peso", ViewType.Point)
        CCPercentilPeso.Series.Add(seriesP)
        ' Generate a data table and bind the series to it.
        seriesP.DataSource = _ds.Tables(0)
        seriesP.ArgumentScaleType = ScaleType.Numerical
        seriesP.ArgumentDataMember = "Argument"
        seriesP.ValueScaleType = ScaleType.Numerical
        seriesP.ValueDataMembers.AddRange(New String() {"Value"})
    End Sub
    Private Sub Percentil(ByVal Sexo As String)

        ' GRAFICA DE TALLA
        ' MAXIMO
        _ds = New DataSet
        _ds = _DPercentil.Listar("AMAXIMO" & Sexo)
        Dim seriesMax As New Series("97", ViewType.Line)
        CCPercentilTalla.Series.Add(seriesMax)
        ' Generate a data table and bind the series to it.
        seriesMax.DataSource = _ds.Tables(0)
        seriesMax.ArgumentScaleType = ScaleType.Numerical
        seriesMax.ArgumentDataMember = "Argument"
        seriesMax.ValueScaleType = ScaleType.Numerical
        seriesMax.ValueDataMembers.AddRange(New String() {"Value"})
        seriesMax.View.Color = Color.Blue

        'NORMAL
        _ds = New DataSet
        _ds = _DPercentil.Listar("ANORMAL" & Sexo)
        Dim seriesNor As New Series("50", ViewType.Line)
        CCPercentilTalla.Series.Add(seriesNor)
        ' Generate a data table and bind the series to it.
        seriesNor.DataSource = _ds.Tables(0)
        seriesNor.ArgumentScaleType = ScaleType.Numerical
        seriesNor.ArgumentDataMember = "Argument"
        seriesNor.ValueScaleType = ScaleType.Numerical
        seriesNor.ValueDataMembers.AddRange(New String() {"Value"})
        seriesNor.View.Color = Color.Green
        'MINIMO
        _ds = New DataSet
        _ds = _DPercentil.Listar("AMINIMO" & Sexo)
        Dim seriesMin As New Series("3", ViewType.Line)
        CCPercentilTalla.Series.Add(seriesMin)
        ' Generate a data table and bind the series to it.
        seriesMin.DataSource = _ds.Tables(0)
        seriesMin.ArgumentScaleType = ScaleType.Numerical
        seriesMin.ArgumentDataMember = "Argument"
        seriesMin.ValueScaleType = ScaleType.Numerical
        seriesMin.ValueDataMembers.AddRange(New String() {"Value"})
        seriesMin.View.Color = Color.Red
        ' GRAFICA DE PESO
        ' MAXIMO
        _ds = New DataSet
        _ds = _DPercentil.Listar("PMAXIMO" & Sexo)
        Dim seriesMaxP As New Series("97", ViewType.Line)
        CCPercentilPeso.Series.Add(seriesMaxP)
        ' Generate a data table and bind the series to it.
        seriesMaxP.DataSource = _ds.Tables(0)
        seriesMaxP.ArgumentScaleType = ScaleType.Numerical
        seriesMaxP.ArgumentDataMember = "Argument"
        seriesMaxP.ValueScaleType = ScaleType.Numerical
        seriesMaxP.ValueDataMembers.AddRange(New String() {"Value"})

        'NORMAL
        _ds = New DataSet
        _ds = _DPercentil.Listar("PNORMAL" & Sexo)
        Dim seriesNorP As New Series("50", ViewType.Line)
        CCPercentilPeso.Series.Add(seriesNorP)
        ' Generate a data table and bind the series to it.
        seriesNorP.DataSource = _ds.Tables(0)
        seriesNorP.ArgumentScaleType = ScaleType.Numerical
        seriesNorP.ArgumentDataMember = "Argument"
        seriesNorP.ValueScaleType = ScaleType.Numerical
        seriesNorP.ValueDataMembers.AddRange(New String() {"Value"})

        'MINIMO
        _ds = New DataSet
        _ds = _DPercentil.Listar("PMINIMO" & Sexo)
        Dim seriesMinP As New Series("3", ViewType.Line)
        CCPercentilPeso.Series.Add(seriesMinP)
        ' Generate a data table and bind the series to it.
        seriesMinP.DataSource = _ds.Tables(0)
        seriesMinP.ArgumentScaleType = ScaleType.Numerical
        seriesMinP.ArgumentDataMember = "Argument"
        seriesMinP.ValueScaleType = ScaleType.Numerical
        seriesMinP.ValueDataMembers.AddRange(New String() {"Value"})
    ...
    Private Sub LimpiarCampos()
        ...
        cboFusionEscrotal.SelectedIndex = 0
        cboMeathusUretral.SelectedIndex = 0
        cboMicroPene.SelectedIndex = 0
        cboTesticuloIZQ.SelectedIndex = 0
        cboTesticuloDER.SelectedIndex = 0
        cboEscalaPrader.Text = ""
    ...
    Private Sub GuardarHistoria()
        ...
        '*********** CAMPOS HISTORIA DETALLE  *********************************************
        _HistoriaDetalle.IdOrden = Val(lblConsecutivoOrden.Text)
        _HistoriaDetalle.IdDetalleOrden = Val(lblIdDetalleOrden.Text)
        _HistoriaDetalle.FuncionEscrotal = cboFusionEscrotal.Text
        _HistoriaDetalle.MicroPene = cboMicroPene.Text
        _HistoriaDetalle.MeatusUretral = cboMeathusUretral.Text
        _HistoriaDetalle.TesticuloIzquierdo = cboTesticuloIZQ.Text
        _HistoriaDetalle.TesticuloDerecho = cboTesticuloDER.Text
        _HistoriaDetalle.IdEscalaPrader = cboEscalaPrader.EditValue
    ...
    Private Sub NuevaHistoria()
        '******************************************************
        ' CAMBIAR PROCESO CARGAR DATOS DE USUARIO
        '******************************************************
        ' 1. PROCESO NUEVA HISTORIA
        lblConsecutivoOrden.Text = _ClickGrilla
        lblIdDetalleOrden.Text = _ClickIdDetalleOrden
        bwNuevo.RunWorkerAsync()
        ppCargar.Visible = True
...
#End Region
    ...
    Private Sub btVistaPrevia_Click(sender As Object, e As EventArgs) Handles btVistaPrevia.Click
        'Proceso anterior - Carlos Martinez 
        'If _ClickGrillaHistoriasAnteriores <> "" Then
        'Dim _frmOpen As New SIM___GLOBAL.frmVistaHistoria
        '_frmOpen.IdDetalleOrden = _ClickGrillaHistoriasAnteriores 'Registro
        '_frmOpen.Licencia = "1" 'Licencia
        '_frmOpen.Estudio = "CONSULTA DE PRIMERA VEZ" 'Estudio
        '_frmOpen.ShowDialog()
        ...
        If _ClickGrillaHistoriasAnteriores <> "" Then
            ...
            _frmOpen.IdDetalleOrden = _ClickGrillaHistoriasAnteriores 'Registro
            _frmOpen.Licencia = 1
            _frmOpen.Estudio = "CONSULTA PRIMERA VEZ"
            _frmOpen.ShowDialog()

        Else
            MessageBox.Show("Debe Seleccionar una Historia", "Vista Previa", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    End Sub
...
#End Region
    ...
    Private Sub bwGuardar_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwGuardar.DoWork
        ...
        '************* GUARDAMOS HISTORIA_DETALLE *********************************************
        _DHistoriaDetalle.Guardar(_HistoriaDetalle)
        ' 1. LOAD - ACTUALIZAR GRILLA
        _dsActualizarGrilla = New DataSet()
        _dsActualizarGrilla = _dordenes.ListarPacientesMartinez
    End Sub
...
#Region "PROCESO NUEVA HISTORIA"
    Private Sub bwNuevo_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bwNuevo.DoWork
        ...
        If _HistoriaDia = True Then
            ...
            ' traemos Historia Detalle
            '_historis = New DataSet()
            _HistoriaDetalle = _DHistoriaDetalle.Cargar(lblIdDetalleOrden.Text)
    ...
    Private Sub bwNuevo_RunWorkerCompleted(sender As Object, e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles bwNuevo.RunWorkerCompleted
        ...
        Else
            ...
            If _HistoriaDia = True Then
                ...
                'traemos Historia_Detalle del DIA por Id_Detalle_Orden
                cboFusionEscrotal.Text = _HistoriaDetalle.FuncionEscrotal
                cboMicroPene.Text = _HistoriaDetalle.MicroPene
                cboMeathusUretral.Text = _HistoriaDetalle.MeatusUretral
                cboTesticuloIZQ.Text = _HistoriaDetalle.TesticuloIzquierdo
                cboTesticuloDER.Text = _HistoriaDetalle.TesticuloDerecho


                cboEscalaPrader.EditValue = Val(_HistoriaDetalle.IdEscalaPrader)
    ...
    Private Sub cboFusionEscrotal_EditValueChanged(sender As Object, e As EventArgs) Handles cboFusionEscrotal.EditValueChanged
        bbiGuardar.Enabled = True
    End Sub

    Private Sub cboMicroPene_EditValueChanged(sender As Object, e As EventArgs) Handles cboMicroPene.EditValueChanged
        bbiGuardar.Enabled = True
    End Sub

    Private Sub cboMeathusUretral_EditValueChanged(sender As Object, e As EventArgs) Handles cboMeathusUretral.EditValueChanged
        bbiGuardar.Enabled = True
    End Sub

    Private Sub cboTesticuloIZQ_EditValueChanged(sender As Object, e As EventArgs) Handles cboTesticuloIZQ.EditValueChanged
        bbiGuardar.Enabled = True
    End Sub

    Private Sub cboTesticuloDER_EditValueChanged(sender As Object, e As EventArgs) Handles cboTesticuloDER.EditValueChanged
        bbiGuardar.Enabled = True
    End Sub
